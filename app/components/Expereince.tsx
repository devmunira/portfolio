...
const Expereince: React.FC<Props> = ({ data }) => {
  return (
    <div className="section">
      <div className="py-14 gap-10 flex mobile:flex-col laptop:flex-row justify-between items-stretch">
        <div className="flex-none laptop:max-w-[5%] mobile:max-w-[100%] mobile:mb-5 mb-0 ">
          <Heading text="Expirence & Job Details" number={3}></Heading>
        </div>

        <div className="flex-none gap-12 py-10 laptop:max-w-[95%] mobile:w-[100%] flex laptop:flex-row flex-wrap mobile:flex-col">
          {data?.expreience?.map((item, index) => (
            <div key={index} className="laptop:w-[45%] mobile:w-[100%]">
